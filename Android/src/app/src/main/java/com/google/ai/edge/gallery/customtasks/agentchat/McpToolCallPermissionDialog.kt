  ...
  AlertDialog(
    ...
    confirmButton = {
      Column(
        ...
        Button(onClick = { onResult(PermissionResult.ALWAYS_ALLOW) }) {
          Text(stringResource(R.string.mcp_tool_always_allow))
        }
        Button(onClick = { onResult(PermissionResult.ALLOW_ONCE) }) {
          Text(stringResource(R.string.mcp_tool_allow_once))
        }
        OutlinedButton(onClick = { onResult(PermissionResult.DENY) }) {
